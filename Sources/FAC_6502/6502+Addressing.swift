...
public extension FAC_6502 {
    func fetchValue(mode: AddressingMode, condition: Bool = false) async -> AddressingValue {
        let byte2 = await next(0x00)
        let byte3 = await next(0x01)
        switch mode {
        case .accumilator:
        ...
        case .absolute:
            ...
            let value = await memoryRead(from: location)
            return AddressingValue(value: value, location: location, cycles: 0, byte2: byte2, byte3: byte3, a: A)
        case .absoluteIndirect:
            // logAsPrint("\(oldPC.hex()): \(opCode.hex()) \(byte2.hex()) \(byte3.hex())")
            PC = PC &+ 2
            let locationLow = await memoryRead(page: byte3, location: byte2)
            let locationHigh = await memoryRead(page: byte3, location: byte2 &+ 1)
        ...
        case .absoluteX:
            ...
            return AddressingValue(value: await memoryRead(from: location), location: location, cycles: cycles, byte2: byte2, byte3: byte3, a: A)
        case .absoluteY:
            // logAsPrint("\(oldPC.hex()): \(opCode.hex()) \(byte2.hex()) \(byte3.hex())")
            PC = PC &+ 2
            let address = wordFrom(low: byte2, high: byte3)
        ...
        case .indirectX:
            ...
            let valueLow = await memoryRead(page: 0, location: byte2Value)
            let valueHigh = await memoryRead(page: 0, location: byte2Value &+ 1)
            let location = wordFrom(low: valueLow, high: valueHigh)
            return AddressingValue(value: await memoryRead(from: location), location: location, cycles: 0, byte2: byte2, byte3: byte3, a: A)
        case .indirectY:
            // logAsPrint("\(oldPC.hex()): \(opCode.hex()) \(byte2.hex())")
            PC = PC &+ 1
            let byte2Value = await memoryRead(page: 0, location: byte2)
            let valueLow = byte2Value &+ Y
            let carry = valueLow < byte2Value ? 1 : 0
            let byte2Value2 = await memoryRead(page: 0, location: byte2 &+ 1)
            let valueHigh = byte2Value2 &+ UInt8(carry)
            let location = wordFrom(low: valueLow, high: valueHigh)
            return AddressingValue(value: await memoryRead(from: location), location: location, cycles: carry, byte2: byte2, byte3: byte3, a: A)
        case .relative:
            // logAsPrint("\(oldPC.hex()): \(opCode.hex()) \(byte2.hex())")
            PC = PC &+ 1
            ...
            if (condition){
                ...
                PC = await relativeJump(twos: twos)
